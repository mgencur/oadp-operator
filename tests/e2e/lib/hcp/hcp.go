...
func (h *HCHandler) GetHostedClusterKubeconfig(hc *hypershiftv1.HostedCluster) (*rest.Config, error) {
	...
	config, err := ReadKubeconfigFromBytes(kubeconfigData)
	if err != nil {
		return nil, err
	}

	RemoveCertificateAuthorityData(config)

	modifiedBytes, err := clientcmd.Write(*config)
	if err != nil {
		return nil, err
	}

	return buildConfigFromBytes(modifiedBytes)
}

// Remove certificate-authority-data from all clusters in the kubeconfig
func RemoveCertificateAuthorityData(config *clientcmdapi.Config) {
...
func (h *HCHandler) DeleteManifestWork(timeout time.Duration) error {
	...
	for _, manifestWorkName := range manifestWorkNames {
		err := wait.PollUntilContextTimeout(h.Ctx, WaitForNextCheckTimeout, HCPBackupTimeout, true, func(ctx context.Context) (bool, error) {
...
func (h *HCHandler) BackupManifestWork(timeout time.Duration) error {
	clusterID, ok := h.HostedCluster.Labels["api.openshift.com/id"]
	if !ok {
		return fmt.Errorf("HostedCluster does not have a label api.openshift.com/id")
	}
